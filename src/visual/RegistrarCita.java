		...
		buscarMedicoPanel.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if(medico != null && medico.isActivo()) {
					...
					JOptionPane.showMessageDialog(null, "El m�dico no existe", "Error", JOptionPane.ERROR_MESSAGE);
					medicoField.setText("");
				}
		...
		agendarPanel.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				if(medicoParaCita == null) {
					JOptionPane.showMessageDialog(null, "Error: M�dico no disponible");
					return;
				}

				if(medicoParaCita.cantCitasDia(fecha) > medicoParaCita.getMaxCitas()){
					JOptionPane.showMessageDialog(null, 
							"El m�dico ha alcanzado el m�ximo de citas para la fecha solicitada");
					return;
				}
	...
	private void reagendarCita() {
		if(validarCampos()) {
			...
			if(Clinica.getInstance().reagendarCita(fecha, codigoCitaReag)) {
				JOptionPane.showMessageDialog(null, "Se ha reagendado la cita con �xito");
				dispose();
			} else {
				JOptionPane.showMessageDialog(null, "No se pudo reagendar la cita");
			}
		} else {
			JOptionPane.showMessageDialog(null, "A�n hay campos por rellenar");
		}
	}
	...
	private void agendarCita(Medico medicoParaCita) {
		if(validarCampos()) {
			...
			if(Clinica.getInstance().hacerCita(cedulaField.getText(), nombreField.getText(), 
				...
				JOptionPane.showMessageDialog(null, "Se ha guardado la cita con �xito");
				dispose();
			} else {
